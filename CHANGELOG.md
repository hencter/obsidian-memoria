...
## v2.4.7（2026-07-24 · 社区插件合规整改与设置兼容修复）

### 修复

- 设置页迁移到 Obsidian 1.13 的声明式设置 API，所有设置均可被全局设置搜索索引
- 保留设置变更后的存储重载、视图刷新和语言即时更新行为
- 瀑布流改为受支持的 CSS Grid 多列布局，不再使用 CSS multicolumn 分栏
- 移除全部 `!important` 声明，并合并重复的原生编辑器样式规则
- 草稿和智能回顾历史保存在插件数据中；标签建议与文件遍历保持在 Motes 存储范围内
- 发布工作流增加 lint、自动生成发布说明和构建产物来源证明
- 为 Obsidian 1.7-1.12 恢复设置页兼容渲染，避免旧版客户端显示空白设置
